...
def nat(id, dummy1, dummy2):
  ...
  while True:
    if len(input_queue[id]) >= 2:
      nat_x = int(input_queue[id].pop(0))
      nat_y = int(input_queue[id].pop(0))

    if idle > 100000:
        idle = 0
        input_queue[0].append(nat_x)
        input_queue[0].append(nat_y)
        print "SENDING: x=" + str(nat_x) + " y=" + str(nat_y)
        if nat_y == last_y:
          print "SEEN TWICE IN A ROW: " + str(nat_y)
          running = 0
          break
        last_y = nat_y
  ...
  while True:
    ...
    if op_code == 4:
      ...
      if output_count == 0:
        ...
        if output_id < 256:
          if output_id == 255:
            print "SENDING TO NAT   x=" + str(output_x) + " ; y=" + str(p1)
...
thread.start_new_thread( nat, (255, '', '') )
